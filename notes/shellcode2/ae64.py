...
class AE64():
    def __init__(self):
        self.alphanum = map(
            ord, list('UVWXYZABCDEFGHIJKLMNOPQRSTabcdefghijklmnopqrstuvwxyz0123456789'))
    ...
    def encode(self, raw_sc, addr_in_reg='rax', pre_len=0, is_rdi_zero=0):
        ...
        while True:
            ...
            # nop len
            if self.encoder_len >= len(final_sc) and self.encoder_len-len(final_sc) <= 6:
                break
        ...
        final_sc += self.nop2*(nop_len/2)+self.nop * \
            (nop_len % 2)+self.enc_raw_sc
        return final_sc
            ...
            for l in self.alphanum:
                ...
                for mul_byte in self.alphanum:
                    if (mul_word*mul_byte) & 0xffff == need:
                        self.mul_cache[need] = (
                            mul_word, mul_byte)  # add to mul cache
